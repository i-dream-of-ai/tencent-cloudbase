...
| `func.triggers[].type` | string | 是 | Trigger type, currently only supports 'timer' | "timer" |  |
...
| `triggers[].type` | string | 是 | Trigger type, currently only supports 'timer' | "timer" |  |
...
| `options` | object |  | 其他选项 |  |  |
| `options.chunkExpand` | array of number |  | 指定返回的文档内容的展开长度,例如 [3,3]代表前后展开长度 |  | [3,3] |
