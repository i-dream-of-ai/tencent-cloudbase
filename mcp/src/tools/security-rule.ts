...
export function registerSecurityRuleTools(server: ExtendedMcpServer) {
  ...
  server.registerTool?.(
    ...
    async ({ resourceType, resourceId, envId }: ReadSecurityRuleParams) => {
      try {
  ...
  server.registerTool?.(
    ...
    async ({ resourceType, resourceId, envId, aclTag, rule }: WriteSecurityRuleParams) => {
      try {
        const cloudbase = await getManager();
        let result;
        if (resourceType === 'database') {
