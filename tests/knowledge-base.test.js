...
test('Knowledge base search functionality works correctly', async () => {
  ...
  try {
    ...
    try {
      ...
      console.log('Knowledge base search result:', JSON.stringify(searchResult, null, 2));
      ...
      if (searchResult.content.length > 0) {
        ...
        console.log('First search result preview:', firstResult.text.substring(0, 200) + '...');
      } else {
        console.log('⚠️ Knowledge base search returned no results (this may be expected if no knowledge base is configured)');
      }
      
    } catch (toolError) {
      console.log('⚠️ Knowledge base search failed (this may be expected if no knowledge base is configured)');
...
test('Knowledge base tool parameters validation', async () => {
  ...
  try {
    ...
    if (!searchTool) {
      console.log('⚠️ Skipping parameter validation - searchKnowledgeBase tool not found');
      return;
    }
